...
El conjunto minimo de operaciones que se definen sobre los árboles son las siguientes: **Crear**, **Destruir**, **Insertar** **Eliminar**, **Buscar**, **Recorrer** y **Vacío**.
...
En mi caso solamente voy a explicar qué son los *Arboles Binarios* y los **ABB**. Como los **ABB** son un tipo de los *Aboles Binarios*, conviene primero explicar que es un *Arbol Binario*. Estos son un tipo de árbol que surge con la idea de aproximarnos a la búsqueda binaria. En este tipo de árbol un nodo puede tener como máximo dos `hijos`. Luego esto nos permite dar una noción de izquierda y derecha dentro del árbol. Es decir, si estamos parados en un nodo, podemos ir a la izquierda o la derecha.
...
---
- La operación de *insertar* un elemento en el árbol, consiste en que debemos recorrer el árbol e ir comparando el elemento que queremos insertar con las claves de los nodos del árbol. El resultado de dicha comparación determinará de qué lado del árbol debemos insertar el elemento.
...
- En cuanto a la operación de *eliminar* un elemento de un árbol, se pueden dar tres situaciones diferentes:
  ...
  ---
  También vemos que en este caso la complejidad de eliminar un elemento es `O(log(n))`, pues el proceso más costoso de todo sería encontrar el elemento que queremos eliminar y este proceso es `O(log(n))`. El resto de las operaciones de eliminar tienen una complejidad constante `O(1)`, pues solamente estamos apuntando punteros.
- Para *buscar* un elemento en el árbol debemos ir comparando el elemento que buscamos con la clave del nodo. Como en el proceso de insertar, esta comparación determina de qué lado del árbol debemos buscar el elemento. Veamos que se tiene una complejidad `O(log(n))`, pues al igual que *insertar* y *eliminar*, lo que hacemos es que a medida que comparamos, partimos el problema a la mitad.
...
Para analizar la complejidad de estas operaciones en un **ABB** se asumio que el árbol estaba balanceado. Que un árbol esté balanceado implica que este no se degenera en una lista. En el caso de que el árbol se degenere en una lista las complejidades de las operaciones pasarían a ser `O(n)`, pues no estaríamos partiendo el problema a la mitad.
